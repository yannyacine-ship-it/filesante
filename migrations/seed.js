...
// Standardize salt rounds to match User model
...
async function seed() {
  ...
  try {
    ...
    // MODIFIED: Now creates nurse accounts in ALL environments (not just development)
    logger.info('Création des comptes infirmières...');
    ...
    for (const hospital of hospitals) {
      ...
      if (existing.length === 0) {
        ...
        logger.info(`Compte nurse existe déjà: ${nurseEmail}`);
      }
    }
    ...
    const { rows: hospitalsForStats } = await db.query('SELECT id FROM hospitals');

    for (const hospital of hospitalsForStats) {
      await db.query(`
